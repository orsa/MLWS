...
public class ImpressionBidder {
		...
		for (PublisherCatalogEntry publisherCatalogEntry : publisherCatalog.getPublishers()) {
			...
			for (Device device : Device.values()) {
				for (AdType adType : AdType.values()) {
					...
					log.info(rname + ": Adding instance - " + attributes.toString());
	...
	private void generateUnknownInstance(String publisherName, Device device, AdType adType, CampaignData urgentCampaign, float leastPriority) {
		...
		double[] attributes = new double[8];
		attributes[0] = publisherStats.getPopularity(); // TODO not sure if popularity should be taken into account here
		attributes[1] = deviceSerialize(device);
		attributes[2] = adTypeSerialize(adType);
		attributes[3] = adTypeOrientation;
		attributes[4] = userAnalyzer.getMarketSegmentWeight(urgentCampaign.getTargetSegment(), publisherName);
		attributes[5] = urgentCampaign.getBudget();
		attributes[6] = thisCampaignPriority * thisCampaignPriority/leastPriority; // Increase priority by how much more urgent it is than the least urgent campaign.
		attributes[7] = NOT_UPDATED;
		
		int[] indicesToFill = new int[8];
		indicesToFill[0] = PUBLISHER_POPULARITY_ATTR_INDEX;
		indicesToFill[1] = DEVICE_ATTR_INDEX;
		indicesToFill[2] = AD_TYPE_ATTR_INDEX;
		indicesToFill[3] = AD_TYPE_ORIENTATION_ATTR_INDEX;
		indicesToFill[4] = WEIGHT_OF_MARKET_SEGMENT_ATTR_INDEX;
		indicesToFill[5] = REMAINING_BUDGET_ATTR_INDEX;
		indicesToFill[6] = PRIORITY_ATTR_INDEX;
		indicesToFill[7] = IS_UPDATED_INDEX;
	...
	private void generateFirstInstance(ImpressionParameters impParams,
		...
		attributes[0] = publisherStats.getPopularity(); // TODO not sure if popularity should be taken into account here
		attributes[1] = deviceSerialize(impParams.getDevice());
		attributes[2] = adTypeSerialize(impParams.getAdType());
		attributes[3] = adTypeOrientation;
		attributes[4] = weight;
		attributes[5] = NOT_UPDATED;
		...
		indicesToFill[3] = AD_TYPE_ORIENTATION_ATTR_INDEX;
		indicesToFill[4] = WEIGHT_OF_MARKET_SEGMENT_ATTR_INDEX;
		indicesToFill[5] = IS_UPDATED_INDEX;
	...
	private double getAdTypeOrientation(PublisherStats publisherStats) {
		return publisherStats.getVideoOrientation() / publisherStats.getTextOrientation();
	}
	...
	private double calcBid(double currentBid, List<CampaignData> relevantCampaigns, String publisherName, Set<MarketSegment> marketSegment) throws Exception {
		...
		for (CampaignData relevantCampaign : relevantCampaigns) {
			...
			double currentBidForInstance = classifier.classifyInstance(campaignInstnace);
	...
	private float getCampaignPriority(CampaignData campaign) { // TODO: check that can't divide by zero
	...
	public void setPublisherStats(
			Map<String, PublisherStats> publisherDailyStats) {
		this.publishersStats = publisherDailyStats;
	}
